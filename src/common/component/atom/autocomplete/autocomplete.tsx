...
export const Autocomplete = (props: AutocompleteProps) => {
  ...
  if (showSuggestions) {
    suggestionsListComponent = (
      <ul className="absolute z-50 w-full mt-1 text-black bg-white divide-y divide-slate-100 rounded-lg shadow-md z-infinity max-h-[15rem] overflow-auto">
  ...
  return (
    <div className={classNames('relative', classNameWrapper)} ref={wrapperRef}>
      <div className="relative flex items-center justify-end w-full" onClick={onClickInput} onKeyDown={(e: any) => onKeyDown(e)}>
        <TextField
          {...inputProps}
          className={classNames('cursor-pointer bg-[#F9F9F9] !select-none text-black focus-visible:outline-none', inputProps.className)}
          ref={inputRef}
