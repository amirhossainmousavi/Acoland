...
export const Representation = () => {
  ...
  const citys =
    provinceName && provinceName !== "همه ایران"
      ? City.filter((item) => item.province === provinceName).map((item) => ({
          label: item.name,
          value: item.name,
        }))
      : City.map((item) => ({ label: item.name, value: item.name }));
  const [representationFormData, setRepresentationFormData] = useState<
    Record<string, RepresentationForm>
  >({
    personaInfo: {
      value: "",
      isValid: true,
  ...
  const handleSubmitForm = () => {
    ...
    if (
      ...
      toast.error(
        ...
        )?.helperText ?? "لطفا شهر و استان خود را انتخاب کنید."
      );
    }
  };
  return (
    <>
        ...
        <div className="w-full md:w-[85%] flex flex-col md:flex-row md:justify-between md:mt-4 gap-2 md:gap-3">
          <div className="w-full items-center md:items-start text-center md:text-start md:w-[50%] flex flex-col gap-3 md:gap-3 md:mt-6">
            ...
            <Text className="block leading-7 text-[#373737]" fontSize="sm">
              ...
              <br />
              آیا می‌خواهید با یک برند معتبر و شناخته‌شده همکاری کنید؟
              <br />
              اگر پاسختان به این سوالات مثبت است، پیوستن به تیم نمایندگی آکولند،
              فرصتی عالی برای شما خواهد بود. آکولند، مرجع تخصصی سرمایه‌گذاری روی
            ...
            <Button
              onClick={handleOpen}
              className="bg-[#35B34A] text-white mt-5"
            >
          ...
          {isDesktop && (
            <div className="w-[40%] p-2  rounded-lg flex justify-end">
              <Image
                src={`${publicRuntimeConfig.LIARA_IMAGE_S3_HOST}/homePage/shutterstock.png`}
                alt="اخذ نمایندگی"
                objectFit="contain"
                width={450}
                height={450}
      ...
      <Modal {...modalProps} noHeader bodyClassName="p-4" className="z-[100]">
        <div className="w-full flex flex-col items-center gap-3">
          ...
          <Text className="text-[#0C0349] text-center w-full">
            ...
            {RepresentationFormData.map((item: any) => (
              <TextField
                ...
                className={classNames(
                  "bg-[#F9F9F9] h-14 text-black focus-visible:outline-none rounded-lg",
                  {
                    "!h-[10rem]": item.isMultiLine,
                  }
                )}
                error={!representationFormData[item.state].isValid}
