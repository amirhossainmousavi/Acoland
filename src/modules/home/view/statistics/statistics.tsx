    ...
    return(
        ...
        <div className='w-full md:w-[85%] flex flex-col md:items-center md:justify-between my-4 md:mt-8 gap-4'>
          <div className='gap-2 md:gap-3 flex flex-col w-full items-center'>
            <Text className='block text-[#08004D]' fontSize={isDesktop ? 'lg' : 'base'}fontWeight='bold'>آمار و دستاوردهای آکولند</Text>
            ...
            {StatisticsData().map((item:any) =>(
              <StatisticsCard key={item.id} title={item.title} description={item.description} image={item.image} />
            ))}
            </div>
          </div>
        </div>
