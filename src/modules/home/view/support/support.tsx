      ...
      <div className="w-full md:w-[85%] flex flex-col md:justify-between md:mt-4 gap-2">
        <div className="flex flex-col items-center w-full text-center md:gap-3 gap-1">
          <Text
            ...
            fontWeight="bold"
          >
            آیا نیاز به راهنمایی یا مشاوره دارید؟
          </Text>
          <Text
            fontSize="sm"
            className="flex flex-col gap-2 text-[#35B34A] [&>span]:block  leading-7"
          >
            <span>
              بدون شک، انتخاب درست یک مشاوره می‌تواند یکی از مهم‌ترین تصمیمات
              مالی زندگی شما باشد
            </span>
            <span>
              پس اگر هنوز نیاز به مشاوره دارید و می‌خواهید از مشاوره‌ حرفه‌ای
              بهره‌مند شوید فرم زیرا تکمیل کنید تا با شما تماس گرفته شود
            </span>
          </Text>
        </div>
        <div className="flex w-full gap-2 items-center">
          {isDesktop && (
            <div className="w-2/4">
              <Image
                src={`${publicRuntimeConfig.LIARA_IMAGE_S3_HOST}/homePage/adsiz.png`}
                alt="پشتیبانی"
                objectFit="contain"
                width={600}
                height={600}
          ...
          <div className=" w-full md:w-2/4 flex flex-col gap-3 ">
            {SupportFromInput.map((item: any) => (
              <TextField
                type={item.type}
                key={item.id}
                classNameWrapper="md:w-[90%] w-full"
                ...
                error={!supportFormData[item.state].isValid}
            ...
            <Button
              className="bg-[#35B34A] text-white  md:w-40 w-full -mt-2 p-8 md:p-0"
