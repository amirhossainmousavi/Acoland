...
export const RepresentationList = () => {
  ...
  const data = isEmpty(searchedRepresentatives)
    ? representativesData
    : searchedRepresentatives;
  ...
  const handleSearchData = (params: any) => {
    ...
    const searchedProjectData = !params
      ? representativesData
      : representativesData.filter((item: any) => {
  ...
  return (
    <div className="md:w-[85%] p-2 md:p-0 flex flex-col overflow-x-auto justify-center items-center mt-4 z-0">
      ...
      >
        استان و شهر و مورد نظر خود را انتخاب کنید تا اطلاعات نماینده های آن شهر
        به شما نشان داده شود. <LocationIcon className="w-12 h-12 md:w-8" />
      </Text>
      ...
      <div className="w-full md:h-[40rem] mt-8 md:mt-4 overflow-x-auto rounded-2xl">
        <div className=" min-w-max md:w-full bg-[#F9F9F9] md:flex-nowrap ">
          <div className="bg-[#08004D] w-full h-24 rounded-t-2xl flex items-center flex-nowrap ">
            ...
            <Text className="w-1/5 block text-white text-center">
              شماره تماس
            </Text>
          </div>
          {isLoading ? (
            ...
            <div className="w-full h-[85%] flex justify-center items-center">
              <Text className="text-gray-400">
                نماینده ای با مشخصات درج شده، یافت نشد!
              </Text>
            </div>
          ) : (
            <div className="md:w-full">
              {data.map((person: any, index: number) => (
                <Representationcard
                  ...
                  phonNumber={person.phonNumber}
                  image={person.image}
                  slug={person.slugProfile}
                />
              ))}
            </div>
          )}
