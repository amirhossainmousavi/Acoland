        ...
        <div className="md:w-[85%] flex flex-col  w-full">
          <div className="flex flex-col md:flex-row justify-between items-start">
            ...
            <div className="flex flex-col gap-2 md:gap-3 md:items-start items-center w-full md:w-[55%] mt-4">
              <Text
                className="text-[#08004D]"
                fontSize={isDesktop ? "lg" : "base"}
                fontWeight="bold"
              >
                {data.gender === "man" ? "آقای" : "خانم"} {data.name}
              </Text>
              <Text
                dangerouslySetInnerHTML={{ __html: data.description }}
                fontSize={isDesktop ? "base" : "sm"}
                className="leading-10 text-[#707070] text-justify"
                fontWeight="light"
              />
            </div>
          </div>
          <div className="flex flex-col items-center md:my-5">
            ...
            <div className="flex flex-col md:flex-row md:flex-wrap w-full h-full gap-3 my-3 md:my-5">
              {data?.project?.map((item: any, index: number) => (
                <div
                  key={index}
                  className="w-full md:w-[49.5%] p-3 border border-[#BBBCBC] border-solid rounded-md"
                >
                  <ProjectCard
                    images={item.images}
...
RepresentativePage.getLayout = function getLayout(
  representativePage: ReactElement
) {
  return (
    <LayoutWithHeaderAndFooter>{representativePage}</LayoutWithHeaderAndFooter>
  );
};
